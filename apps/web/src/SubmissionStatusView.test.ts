...
describe('SubmissionStatusView', () => {
  ...
  /**
   * The live draft card. Its whole reason to exist is the stretch where an agent is deep
   * in details and the thread is only sentences — so what matters is that it appears
   * while a build is running, sits above the composer rather than on it, and goes away
   * where a running draft would describe nothing.
   */
  describe('the live draft card', () => {
    it('runs the channel draft in the thread while the build is still going', async () => {
      (globalThis as typeof globalThis & { IS_REACT_ACT_ENVIRONMENT?: boolean }).IS_REACT_ACT_ENVIRONMENT = true;
      mockedGetSubmissionStatus.mockResolvedValue({
        status: 'building',
        slug: 'comet-courier',
        playable: [{ ref: 'p1', slug: 'comet-courier', label: 'Live preview' }],
      });
      mockedGetChannelPlayable.mockResolvedValue(
      ...
      await act(async () => {
        root.render(createElement(SubmissionStatusView, { token: 'live-draft-token', embedded: true }));
        await flushEffects();
        await flushEffects();
      });

      const card = container.querySelector('[data-testid="studio-live-preview"]');
      expect(card).not.toBeNull();
      // Inside the foot: that block is what the composer's height is made of, so the
      // card is anchored above the reply box instead of floating over it.
      expect(container.querySelector('.studio-thread-foot [data-testid="studio-live-preview"]')).not.toBeNull();
      expect(card!.querySelector('iframe')?.getAttribute('sandbox')).toBe('allow-scripts allow-pointer-lock');
    ...
    it('stays out of the way on a published game, where the draft describes nothing', async () => {
      (globalThis as typeof globalThis & { IS_REACT_ACT_ENVIRONMENT?: boolean }).IS_REACT_ACT_ENVIRONMENT = true;
      mockedGetSubmissionStatus.mockResolvedValue({
        status: 'published',
        slug: 'comet-courier',
        playable: [{ ref: 'p1', slug: 'comet-courier' }],
      });
      mockedGetChannelPlayable.mockResolvedValue(
        '<!doctype html><html><body><canvas id="game"></canvas></body></html>',
      );
      ...
      await act(async () => {
        root.render(createElement(SubmissionStatusView, { token: 'published-token', embedded: true }));
        await flushEffects();
        await flushEffects();
      });

      expect(container.querySelector('[data-testid="studio-live-preview"]')).toBeNull();
    ...
    it('keeps the draft up on a bounce — it is what the creator is deciding about', async () => {
      (globalThis as typeof globalThis & { IS_REACT_ACT_ENVIRONMENT?: boolean }).IS_REACT_ACT_ENVIRONMENT = true;
      mockedGetSubmissionStatus.mockResolvedValue({
      ...
      await act(async () => {
        root.render(createElement(SubmissionStatusView, { token: 'bounced-token', embedded: true }));
        await flushEffects();
        await flushEffects();
      });

      expect(container.querySelector('[data-testid="studio-live-preview"]')).not.toBeNull();
