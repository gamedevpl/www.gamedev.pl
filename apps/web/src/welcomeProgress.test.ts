...
describe('welcomeProgressMessage', () => {
  it('prefers the latest agent event text', () => {
    const status = {
      status: 'building',
      events: [{ id: '1', kind: 'step', text: ' Laying out the bastion ', createdAt: '2026-08-07T00:00:00Z' }],
    } as SubmissionStatus;
  ...
  it('falls back through note, presence, phase, then state copy', () => {
    ...
    expect(welcomeProgressMessage({ status: 'queued', phase: 'dispatched' }, t)).toBe(
      'statusView.phases.dispatched',
    );
...
describe('welcomeStatusLabel', () => {
  it('uses the phase label when the agent is starting', () => {
    expect(welcomeStatusLabel({ status: 'queued', phase: 'dispatched' }, t)).toBe(
      'statusView.phaseLabels.dispatched',
    );
