...
// The delivery allowlist itself (fixed files, extra-module pattern, reserved segments)
// lives in `@gamedevpl/contract` so the CLI can keep undeliverable files local instead
// of failing a whole delivery on them. It is still this side of the lockstep: CI diffs
// it against the games repo exactly as before.
export { DELIVERY_EXTRA_MODULE_PATTERN, DELIVERY_FIXED_FILES, DELIVERY_RESERVED_SEGMENTS } from '@gamedevpl/contract';
