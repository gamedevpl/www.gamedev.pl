...
import { mediaRedirectPlan, type MediaUrlSigner } from '../delivery/media-url-signer.js';
import {
...
export async function registerCatalogRoutes(
  ...
  // Mirrors createCombinedPublishedSlugGate, which is env-built; see published-slugs.ts.
  async function isSlugPublishedAnyLane(slug: string): Promise<boolean> {
    // Store first, as /play does, so repo stalls cannot delay it.
    if (store) {
      try {
        if (isPublished(await store.getPublication(slug))) return true;
      } catch {
        // A transient store failure must not turn a report into a 500.
      }
    }
    try {
      return await isSlugPublished(slug);
    } catch {
      // A repo-lane outage fails closed rather than into a 500.
      return false;
    }
  }
  ...
  async function redirectToSignedMedia(
    ...
    try {
      const plan = mediaRedirectPlan(filename, now());
      const signed = await signer.urlFor(object, plan.ttlSeconds, plan.signedAtMs);
      if (!signed) return false;
      // One sample decides both; a roll mid-await cannot split them.
      reply.header('Cache-Control', `public, max-age=${plan.maxAgeSeconds}`).redirect(signed, 302);
      return true;
    } catch (error) {
