...
it.each(['claim', 'legacy'])(
  'serializes %s binding against recovery through the shared game document',
  async (mode) => {
    ...
    const results = await Promise.all([
      store.claimSubmissionSlug(1, 'sky', null, { key: 'key', spec: 'local', locale: 'en' }),
      mode === 'claim'
        ? store.claimSubmissionSlug(2, 'sky', null)
        : store.setSubmissionSlug(2, 'sky').then(
            () => true,
            () => false,
          ),
    ]);
