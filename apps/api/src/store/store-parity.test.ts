...
describeStoreContract('proposal posting', (makeStore) => {
  ...
  it('names a moved delivery apart from a card that already posted', async () => {
    const store = makeStore();
    await claimed(store);
    // The claim still holds; only the delivery moved under it.
    await store.setSubmissionPreviewVersion(11, 'v2');
    ...
    ).toEqual({ posted: null, refusedBy: 'version' });
  });
