...
export const BEHAVIOURAL_CONTRACT = [
  ...
  'With a shell, screenshot as soon as the game draws: launch headless Chromium with --use-gl=angle --use-angle=swiftshader-webgl --enable-unsafe-swiftshader --enable-webgl --ignore-gpu-blocklist (never --disable-gpu), capture canvas.toDataURL, keep PNG ≤700 KB, then screenshot_upload_url + curl --upload-file. A black frame means those flags were missing; GAME_CAPTURE_GFX=canvas2d / ?gfx=canvas2d is the software fallback. Without a shell or browser, skip mid-build screenshots — deliver mode=preview and read frames via get_gate_media. There is no base64 screenshot tool — PNG bytes must never enter the model.',
