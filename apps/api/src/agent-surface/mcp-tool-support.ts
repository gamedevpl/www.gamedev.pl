...
export const BEHAVIOURAL_CONTRACT = [
  ...
  'With a shell, screenshot as soon as the game draws: launch headless Chromium with --use-gl=angle --use-angle=swiftshader-webgl --enable-unsafe-swiftshader --enable-webgl --ignore-gpu-blocklist (never --disable-gpu), capture canvas.toDataURL in the same render callback (or page.screenshot({path:"shot.png"}); preserveDrawingBuffer:true only in a disposable capture harness, never in shipped game source), decode a data URL with Buffer.from(dataUrl.split(",")[1],"base64") to shot.png in-process (never print or return it), keep PNG ≤700 KB, then screenshot_upload_url and run its `upload` one-liner verbatim (curl -H "Authorization: Bearer <upload token>" -H "Content-Type: image/png" --upload-file shot.png "$url"). A black frame means those flags were missing or the drawing buffer was discarded; GAME_CAPTURE_GFX=canvas2d / ?gfx=canvas2d is the software fallback. Without a shell or browser, skip mid-build screenshots — deliver mode=preview, end, and on a later/resumed run call get_gate_verdict once then get_gate_media if a preview verdict is already available. There is no base64 screenshot tool — PNG bytes must never enter the model.',
  'While iterating, deliver with mode=preview (no TRACE required). Prefer batch stage_upload_url({ paths: [...] }) + the `upload` one-liner it returns (it carries the Content-Type header) for new/rewritten paths when you have shell (bytes never re-enter the model; ALWAYS mint URLs in batch with paths: [...] up to 50 paths per call, chunking into batches of 50 if staging more, rather than emitting individual stage_upload_url calls; use stage_upload_url({ path }) only for a lone file). stage_source_file is the no-shell fallback. Prefer patch_source_file for edits — prefer old+new exact replace, or files: [{ path, old, new }, ...] to edit several files in one call; patch=unified diff also works (never re-emit a whole large render.ts/model.ts). To retire a path (an old game/*.ts module, or a hand-authored index.html/GAME.json field), call delete_source_file — staging empty content still delivers a live empty file, not a removal. Honour warnings.code=module_too_large by splitting before more feature work. Then submit_sources({ fromStaged:true, mode:"preview", kitEngineRef }) — fromStaged overlays onto the latest delivery/seed so only changed paths need staging. Avoid one giant files[] payload. Only mode=publish needs TRACE/PLAYTEST and can go green.',
