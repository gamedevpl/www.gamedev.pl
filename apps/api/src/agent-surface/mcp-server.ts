...
const SESSION_WORKFLOW: readonly string[] = [
  ...
  'Screenshots: without a shell or browser, skip mid-build shots — deliver mode=preview then end; get_gate_media only in a later/resumed run once a preview verdict is already available (the gate captures with WebGL flags; do not call it right after submit). With a shell: launch headless Chromium with --use-gl=angle --use-angle=swiftshader-webgl --enable-unsafe-swiftshader --enable-webgl --ignore-gpu-blocklist (never --disable-gpu; Chrome ≥150 may need --use-angle=swiftshader). Capture canvas.toDataURL("image/png") inside the same render callback (or getContext("webgl",{preserveDrawingBuffer:true}); after compositing the default buffer is gone) — page.screenshot()/CDP compositor also works. Keep PNG ≤700 KB, then screenshot_upload_url and curl --upload-file <png> "$url". A black/blank frame means those WebGL flags were missing or the drawing buffer was already discarded. If SwiftShader is unavailable, GAME_CAPTURE_GFX=canvas2d or ?gfx=canvas2d (force2d). There is no base64 send path — PNG bytes must never enter the model.',
