...
function channelDelivery(brief: BuildBrief, creating: boolean): string[] {
  return [
    ...
    'Before the first preview, stage `game.ts`, `GAME.json`, and the complete editor bundle: compiled `EDITOR.json`, optional authoring `EDITOR.ts`, `EDITOR.content.json` for v2, and `game/editor-content.ts`.',
    'Do not submit until the editor declaration and generated content are staged with the playable source.',
    ...
    '- GAME.json must include an engine.modules array; every game needs an EditorKit editor with at least three meaningful tunables or one content collection; stage compiled `EDITOR.json`, optional `EDITOR.ts`, `EDITOR.content.json` (v2), and `game/editor-content.ts` before submit; run editor:gen when EDITOR.ts is present.',
