...
import { headerOnlyPng, pngHeader } from '../platform/image-size.test.js';
...
describe('agent concept frame slots', () => {
  ...
  it('names the mute rather than sending the agent off to deliver first', async () => {
    ...
    const store = new InMemoryStore();
    await seed(store);
    // No capture yet either; the permanent answer still outranks the transient one.
    app = await createApp(store, stubGamesStore(null));
    await store.setProposalsMuted('g:owner', '2026-09-07T12:00:00.000Z');
  ...
  it('names the switch rather than sending the agent off to deliver first', async () => {
    const store = new InMemoryStore();
    await seed(store);
    app = await createApp(store, stubGamesStore(null));
  ...
  it('refuses a concept frame that measures fine and carries no pixels', async () => {
    vi.stubEnv('AGENT_PROPOSALS_ENABLED', 'true');
    const store = new InMemoryStore();
    await seed(store);
    app = await createApp(store, stubGamesStore());

    const put = await uploadConceptFrameRaw(app, headerOnlyPng(900, 900));

    // The aspect ratio matches; only the pixels are missing.
    expect(put.json().rejected).toBe('frame_blank');
    expect(await store.countBuildShots(ISSUE)).toBe(0);
  });

  it('refuses a pixel-less frame stored before the upload guarded it', async () => {
    vi.stubEnv('AGENT_PROPOSALS_ENABLED', 'true');
    const store = new InMemoryStore();
    await seed(store);
    app = await createApp(store, stubGamesStore());
    const frames = [
      ...
      (
        await store.appendBuildShot(ISSUE, {
          data: headerOnlyPng(900, 900).toString('base64'),
    ...
    expect(response.json().rejected).toBe('frame_blank');
    // A refused frame must not spend the delivery's one claim.
