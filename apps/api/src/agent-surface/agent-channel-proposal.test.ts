...
describe('agent-written concept proposals', () => {
  ...
  it('says ask again when the delivery moves after the claim, not `already_proposed`', async () => {
    vi.stubEnv('AGENT_PROPOSALS_ENABLED', 'true');
    const store = new InMemoryStore();
    await seed(store);
    app = await createApp(store, stubGamesStore());
    ...
    const real = store.appendBuildShot.bind(store);
    store.appendBuildShot = async (jobId, shot) => {
      // Past the claim: only the posting transaction can still refuse.
      await store.setSubmissionPreviewVersion(ISSUE, 'v8');
      return await real(jobId, shot);
    };

    const response = await propose(app, frames);
    // The new delivery is unclaimed; the agent may still draw.
    expect(response.json().rejected).toBe('frame_stale');
    expect(await store.listCreatorMessages(ISSUE)).toHaveLength(0);
  });

  it('takes back the capture it wrote when the card is refused', async () => {
    vi.stubEnv('AGENT_PROPOSALS_ENABLED', 'true');
    const store = new InMemoryStore();
    await seed(store);
    app = await createApp(store, stubGamesStore());
    const frames = [await storeConceptFrame(store), await storeConceptFrame(store)];
    const before = await store.countBuildShots(ISSUE);
    const real = store.appendBuildShot.bind(store);
    store.appendBuildShot = async (jobId, shot) => {
      await store.setSubmissionPreviewVersion(ISSUE, 'v8');
      return await real(jobId, shot);
    ...
    expect((await propose(app, frames)).json().rejected).toBe('frame_stale');
    // The agent's frames stay; the capture we wrote does not.
    expect(await store.countBuildShots(ISSUE)).toBe(before);
  });
