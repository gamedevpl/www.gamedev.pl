...
export type ProposalRefusal =
  ...
  | 'frame_stale'
  | 'frame_shape'
  // A frame stored before the upload guard, or one gone blank.
  | 'frame_blank'
...
const REFUSAL_OF: Record<ProposalRefusedBy, ProposalRefusal> = {
  ...
  muted: 'muted',
  round: 'frame_stale',
  // The delivery moved; the new one is unclaimed, not already carded.
  version: 'frame_stale',
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      for (const frame of frames) {
        ...
        // The upload guards this too; a stored frame predates it.
      ...
      if (result.posted === null) {
        // Ours to undo; the agent's own frames are not.
        await store
          .deleteBuildShots(jobId, [sourceShot.id])
          .catch((error: unknown) =>
            request.log.error(
              { err: error, jobId, shots: [sourceShot.id] },
              'proposal shots orphaned; delete these ids by hand',
            ),
          );
