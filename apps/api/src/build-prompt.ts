...
function channelDelivery(brief: BuildBrief, fast: boolean): string[] {
  if (fast) {
    return [
      ...
      '- Always call `.audio()` in GameKit.defineGame; select valid audio.sounds in GAME.json.',
      '- Use existing sound ids from the template or digest; never invent audio ids.',
