...
describe('assessment attribution at the HTTP boundary', () => {
  ...
  it.each(['canonical', 'erased', 'catalog', 'unknown'])('ignores forged attribution for %s games', async (kind) => {
    ...
    const app = await buildApp({
      ...
      reviewRoutes: {
        listCatalog: async () =>
          kind === 'unknown' ? [] : [{ slug: 'sky', title: 'Sky', creatorHandle: 'catalog-author' }],
      },
    });
    apps.push(app);
