...
export function buildPrompt(brief: BuildBrief): string {
  ...
  const lines = [
    ...
    '`npm run progress -- --check` after each meaningful step and at least every few minutes',
    'while you are heads-down editing — silence here is how their note sits unread until you',
    'finish the wrong thing. `--ack <id>` only after you have actually acted on a message.',
    'Stop immediately when `stop` is set.',
    '',
