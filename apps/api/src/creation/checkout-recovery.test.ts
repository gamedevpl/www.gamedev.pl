...
it.each(['canceled', 'abandoned'] as const)(
  'rejects a %s retry key and allows fresh recovery without losing history',
  async (state) => {
    const f = await fixture();
    const body = payload();
    ...
    await f.store.recordJobTransition(recovered.jobId, {
      ...
      by: 'operator',
      reason: state === 'canceled' ? 'operator_canceled' : 'self_build_quiet_timeout',
    });
    ...
    expect((await f.store.getSubmission(recovered.jobId))?.state).toBe(state);
  },
);
