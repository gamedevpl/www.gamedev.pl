...
describe('publishing a bot-seeded game transferred to a creator', () => {
  ...
  it('refuses publish when game ownership changes during clearance', async () => {
    const store = new InMemoryStore();
    const at = new Date().toISOString();
    await store.upsertUser({ uid: 'g:boss' });
    await store.upsertUser({ uid: 'bot:seeder' });
    ...
    const gamesStore = {
      ...
      getManifest: async () => {
        ...
        return {
          ...
          sourceFiles: ['SPEC.md'],
          deliveryMode: 'publish' as const,
          gate: { green: true, ranAt: '2026-07-30T11:00:00Z' },
        };
      },
    } as unknown as GamesStore;
    ...
    try {
      ...
      expect(response.statusCode).toBe(409);
      expect(response.json()).toMatchObject({ error: 'owner_changed' });
      expect(await store.getPublication('sky-dodge')).toBeNull();
    } finally {
      await app.close();
    }
