...
import { headerOnlyJpeg, headerOnlyPng, jpegHeader, pngHeader, truncated } from '../platform/image-size.test.js';
...
describe('createDreamJob', () => {
  ...
  it('posts no card from a frame that measures fine but has no pixels', async () => {
    const { store, run } = await harness({
      hud: [],
      // Markers and dimensions, no scan data: a browser draws nothing.
      frame: { data: headerOnlyJpeg(1024, 1024).toString('base64'), mediaType: 'image/jpeg' },
    });
  ...
  it('posts no card from a PNG frame with no IDAT', async () => {
    const { store, run } = await harness({
      hud: [],
      frame: { data: headerOnlyPng(1024, 1024).toString('base64'), mediaType: 'image/png' },
    });
