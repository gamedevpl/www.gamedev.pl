...
async function main() {
  ...
  if (process.env.NODE_ENV !== 'production' && process.env.DEV_SEED_STUDIO === '1' && store instanceof InMemoryStore) {
    ...
    // Local-only demo shots so Creator Studio can show dismissable screenshot toasts.
    ...
    const demoShotPng = (paint: (x: number, y: number, w: number, h: number) => [number, number, number]) => {
      const w = 96;
      const h = 60;
