...
export function createGitHubClient(options: GitHubClientOptions): GitHubClient {
      ...
      plugins: [
        {
          ...
          setup(builder) {
            ...
            builder.onLoad({ filter: /.*/, namespace: 'github-typescript-source' }, async (args) => {
              ...
              let source =
                overrideOf(loadedPath) ??
                (options?.noRefFallback ? null : await readRawFile(loadedPath.slice(1), ref));
  ...
  function bundleGameTypeScript(
    ...
    return bundleTypeScriptGraph(entrySource, ref, gameRoot, `${gameRoot}/game.ts`, 'game', overrides, collect, options);
  }
