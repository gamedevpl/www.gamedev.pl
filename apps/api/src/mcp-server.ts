...
const SESSION_WORKFLOW: readonly string[] = [
  ...
  'Staging is already visible: once game.ts, style.css, GAME.json and markup are present across staging + delivery/seed, the platform assembles a live playable preview — without waiting for submit or the gate. Markup means either an index.html or a GAME.json howToPlay carrying goal and hint, from which the body is generated. Stage a runnable tree early and keep staging/patching as you work; a buffer that does not compile simply leaves the previous preview up.',
