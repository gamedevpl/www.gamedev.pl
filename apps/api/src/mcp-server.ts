...
const SESSION_WORKFLOW: readonly string[] = [
  ...
  'get_sources — always, and before any scaffolding decision. available:true means this game has files: origin=seed is a generated round-0 draft for a new game, origin=delivery is what a previous round delivered. Continue those files either way; never scaffold over them. seedStatus=pending means a draft is still generating — call again before falling back to `npm run create`. If warnings.code=module_too_large, split those oversized modules into cohesive game/*.ts pieces BEFORE adding features — do not grow them further.',
  ...
  > = {
    ...
    get_seed: {
      ...
      description:
        ...
        'Only scaffold (`npm run create`, which copies one of the kit starters/ games) when status=unavailable; that response explicitly says no seed exists for this round. ' +
