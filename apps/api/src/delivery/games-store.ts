...
export function validateSourceUpload(
  ...
  if (requireEditorJson && !seen.has('EDITOR.json')) {
    throw new InvalidUploadError(
      'EDITOR.json is required for a new game — every newly created game must be editable',
      undefined,
      ['EDITOR.json'],
    );
  }
