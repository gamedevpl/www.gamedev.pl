...
function channelDelivery(brief: BuildBrief, creating: boolean): string[] {
  return [
    ...
    '- GAME.json must include an engine.modules array; copy the shape from a starter and never omit it. Every game must ship an EditorKit editor: prefer EDITOR.ts or provide EDITOR.json with at least three meaningful tunables or one content collection; keep generated editor content in sync and stage it with the game source.',
