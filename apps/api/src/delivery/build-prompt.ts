...
function channelDelivery(brief: BuildBrief, creating: boolean): string[] {
  return [
    ...
    'Before the first preview, stage `game.ts`, `GAME.json`, and the complete editor bundle: `EDITOR.ts` (preferred, or `EDITOR.json`), `EDITOR.content.json` for v2, and `game/editor-content.ts`.',
    ...
    '- GAME.json must include an engine.modules array; every game needs an EditorKit editor with at least three meaningful tunables or one content collection; stage `EDITOR.ts`, `EDITOR.json`, `EDITOR.content.json` (v2), and `game/editor-content.ts` before submit.',
