...
it('refuses missing GameKit modules before candidate storage or Cloud Build', async () => {
  ...
  await expect(
    service.deliver({
      ...
      files: [
        ...
        { path: 'GAME.json', content: JSON.stringify({ engine: { modules: ['input', 'gfx', 'audio'] } }) },
      ],
    }),
  ).rejects.toThrow(/add \["effects"\]/);
