...
it('/retry resumes a failed delegate with its original request and partial edits', async () => {
  ...
  const ws: Workshop = {
    ...
    runAdapter: async (input) => {
      ...
      if (prompts.length === 1) {
        writeFileSync(join(input.cwd, 'game.ts'), 'partial');
        return { code: 1 };
      }
      expect(readFileSync(join(input.cwd, 'game.ts'), 'utf8')).toBe('partial');
      writeFileSync(join(input.cwd, 'game.ts'), 'finished');
      return { code: 0 };
    },
  };
  const write = (line: string) => lines.push(line);
  await handleReplLine({ line: '/delegate add three tracks', api, token: 'tok', workshop: ws, write });
  expect(ws.failedTask).toEqual({ request: 'add three tracks', ack: undefined, agent: 'codex' });
  ...
  expect(ws.failedTask).toBeUndefined();
