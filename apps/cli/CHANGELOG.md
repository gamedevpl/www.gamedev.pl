...
- `status` inside a checkout lists the working copy, and `diff` prints a patch against the platform.
- Checkouts skip `.git`, follow `.gitignore`, and also read `.gamedevplignore`. Push names ignored files it left out. Pull refuses when an ignored file on disk would be overwritten, and leaves a platform file unwritten when it is not already in the checkout.
