...
- Local headless agents run through `genaicode/agents` 2.5.0: its drivers decode each agent's events, the per-vendor event readers are gone, and the verify-and-repair loop uses `runWithVerify`.
