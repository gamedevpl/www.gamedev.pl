...
| `MCP_AUTHORIZATION_SERVERS` (optional) | Comma-separated issuer URL(s) for the MCP OAuth authorization server — advertised in `/.well-known/oauth-protected-resource` when set. Set to the canonical app origin (e.g. `https://www.gamedev.pl`) once BY-18b is deployed. | unset locally                                                                                       |
