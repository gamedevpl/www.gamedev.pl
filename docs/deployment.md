...
| `option-image-api-key`                 | CreatorQA option tiles → `OPTION_IMAGE_API_KEY` (needs `OPTION_IMAGE_MODEL` set too) — not provisioned. Deliberately NOT `meta-api-key`: this path sends real creator concepts and option labels to the vendor, which ops: legal-compliance-plan.md gates separately from round-0 seeding (its items 1–2 must close first) | not set                                                                                             |
