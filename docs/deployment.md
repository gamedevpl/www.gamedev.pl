...
| `meta-api-key`                         | Round-0 seed provider credential → `SEED_META_API_KEY`, and CreatorQA option tiles → `OPTION_IMAGE_API_KEY` (Muse Image; needs `OPTION_IMAGE_MODEL` set too, ops: creatorqa-option-images-spike.md) — not provisioned; gated on the wave-3 legal/credential steps in seed-provider-selection-plan.md | not set                                                                                             |
