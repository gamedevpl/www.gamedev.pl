...
_as_ `gamedev-app@` or `erase-verifier@` and execute with those accounts' Firestore
...
| `GAMES_REPO_TOKEN` (GitHub Actions)    | Contents:read PAT on the games repo — validation checkout, master-only CI lockstep check (`npm run contract:games-repo`), and snapshot bake (`publish-games.yml`)                                                                                                                    | ⚠️ set on the GitHub repo (not GCP) so assemble/Check 4/music drift fails trusted post-merge CI     |
...
| `openai-api-key`                       | OpenAI managed-agent credential → `OPENAI_API_KEY`; same secret also wired to `SEED_OPENAI_API_KEY` (round-0 seeding, ops: seed-provider-selection-plan.md) and to `OPTION_IMAGE_API_KEY` (CreatorQA option tiles, owner decision 2026-09-23; off until `OPTION_IMAGE_MODEL` is set) | optional                                                                                            |
...
_minting_. They no longer bound _volume_: one 302 is a six-hour URL that Cloud Storage
...
`/invite/<code>` links remain supported and still send their code in the request path.
Revoke and replace old links if their request logs may have been exposed.
