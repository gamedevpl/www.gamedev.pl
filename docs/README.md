...
| Device sensing  | ✅ Tilt play — the shell reads `deviceorientation` on its own origin and relays a clamped stick into games that ask (`apps/web/src/sensing.ts`; games-repo `docs/camera-ar-platform.md` Phase 0). Readings never leave the browser; the iframe sandbox is unchanged |
