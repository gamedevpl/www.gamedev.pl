...
| Orchestration   | 🗃️ Self-hosted agent execution abandoned for legal reasons (see `games-repo.md`); 🚧 hosted Anthropic managed builder (MCP + per-round vault) is implemented, but production selection needs valid configuration and environment-registry wiring (`managed-agent-backend.md`) |
