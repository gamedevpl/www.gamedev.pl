...
| Orchestration   | 🗃️ Self-hosted agent execution abandoned for legal reasons (see `games-repo.md`); ✅ hosted Anthropic managed builder (MCP + per-round vault) is live when `MANAGED_AGENT_VENDOR` is set (`managed-agent-backend.md`)                                              |
