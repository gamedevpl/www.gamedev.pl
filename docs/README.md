...
| Orchestration   | 🗃️ Self-hosted agent execution abandoned for legal reasons (see `games-repo.md`); ✅ hosted Anthropic managed builder (MCP + per-round vault) is live when valid configuration selects it through the environment registry (`managed-agent-backend.md`). Opt-in MCP `ownership` scope can read current-member access and prepare a Studio transfer proposal; the agent cannot complete a transfer. |
...
| [`firestore-read-cost.md` (ops repo)](https://github.com/gamedevpl/www.gamedev.pl-ops/blob/main/docs/firestore-read-cost.md) | ➡️ **Moved 2026-09-20** to the private ops repo — read volumes, per-route costs and the read alerts. The gate stays here: `npm run firestore-read-cost` |
...
| [`catalog-effort.md`](./catalog-effort.md)                                                             | 0..1 bake-time effort score mixed into Recommended ranking                                                  |
