import { PromptItem } from 'genaicode/dist/ai-service/common';
...
export async function generateMasterplan(
  ...
  const { generateContent } = await import('genaicode/dist/ai-service/chat-gpt.js');
  const [saveMasterplan] = await generateContent(
    [
      {
        type: 'user',
        text: `Your task is to generate a masterplan for a battle, please adhere to the following requirements:
...
Your goal is to create a masterplan that can counter the "${resultChain[0].name}" plan:
