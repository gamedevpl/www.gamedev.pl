...
export const units: Unit[] = [
  ...
  { id: 1101, col: -6, row: 2, sizeCol: 6, sizeRow: 2, type: 'warrior', command: 'advance' },
  { id: 1102, col: 6, row: 2, sizeCol: 6, sizeRow: 2, type: 'warrior', command: 'advance' },

  // Left and right shell (Warriors) - reduced from 4 to 2 units
  { id: 1103, col: -8, row: -2, sizeCol: 6, sizeRow: 2, type: 'warrior', command: 'advance' },
  { id: 1104, col: 8, row: -2, sizeCol: 6, sizeRow: 2, type: 'warrior', command: 'advance' },

  // Rear shell (Warriors) - reduced from 3 to 2 units
  { id: 1105, col: -6, row: -6, sizeCol: 6, sizeRow: 2, type: 'warrior', command: 'advance' },
  { id: 1106, col: 6, row: -6, sizeCol: 6, sizeRow: 2, type: 'warrior', command: 'advance' },

  // Internal tanks for breakthrough capability - kept 2 units
  { id: 1107, col: -3, row: -2, sizeCol: 3, sizeRow: 3, type: 'tank', command: 'advance' },
  { id: 1108, col: 3, row: -2, sizeCol: 3, sizeRow: 3, type: 'tank', command: 'advance' },

  // Internal archers for ranged support - kept 2 units
  { id: 1109, col: -4, row: -4, sizeCol: 3, sizeRow: 2, type: 'archer', command: 'advance' },
  { id: 1110, col: 4, row: -4, sizeCol: 3, sizeRow: 2, type: 'archer', command: 'advance' },

  // Central artillery for long-range support - kept 1 unit
  { id: 1111, col: 0, row: -4, sizeCol: 4, sizeRow: 2, type: 'artillery', command: 'advance' },
];
...
export const probabilityScore = (playerUnits: Unit[]): number => {
  ...
  return (
    (meleeUnitScore * 0.25 +
      ...
      largeUnitScore * 0.15 +
      lowArtilleryScore * 0.2 +
      lowRangedScore * 0.1 +
      frontConcentrationScore * 0.1) *
    100
  );
};