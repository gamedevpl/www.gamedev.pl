...
function generatePrompt() {
  ...
  - \`// CODEGEN START: some hints for you\`
  - \`// CODEGEN END\`

  For example this fragment:
  \`\`\` 
  console.log('Some other statement');
  // CODEGEN START: print hello world to console
  // CODEGEN END
...
const codeGenFiles = Object.entries(sourceCode)
  .filter(([path, content]) => content.includes('// CODEGEN') && !path.includes('prompt-gen.js'))
  .map(([path]) => path);
...
export const codeGenPrompt = `Generate updates only for the following files:
${codeGenFiles.join('\n')}
