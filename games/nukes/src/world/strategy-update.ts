...
export function strategyUpdate(worldState: WorldState): WorldState {
  ...
  for (const state of worldState.states) {
    const isStateHostile = Object.entries(state.strategies).some(
      ([stateId, strategy]) =>
        strategy === Strategy.HOSTILE && worldState.launchSites.some((launchSite) => launchSite.stateId === stateId),
    );

    if (!isStateHostile) {
      const stateLaunchSitesCount = worldState.launchSites.filter((site) => site.stateId === state.id).length;
      const candidates = worldState.states.filter(
        (candidateState) =>
          candidateState.id !== state.id &&
          state.strategies[candidateState.id] === Strategy.NEUTRAL &&
          worldState.launchSites.filter((site) => site.stateId === candidateState.id).length < stateLaunchSitesCount &&
          worldState.cities
            .filter((city) => city.stateId === candidateState.id)
            .some((city) => city.populationHistogram.slice(-1)[0].population > 0),
      );
