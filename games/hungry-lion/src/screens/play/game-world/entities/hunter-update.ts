...
import { calculateWrappedDistance, vectorAngleBetween, vectorNormalize, vectorSubtract } from '../utils/math-utils'; // Import calculateWrappedDistance
...
export function detectLion(hunter: HunterEntity, updateContext: UpdateContext): LionEntity | undefined {
  ...
  for (const lion of lions) {
    // Use calculateWrappedDistance for detection range check
    ...
    if (distance <= effectiveRange) {
      // Calculate direction to lion (Note: direction calculation might need wrapping if used for movement)
...
export function shootLion(hunter: HunterEntity, lion: LionEntity, updateContext: UpdateContext): boolean {
  ...
  // Adjust for distance using calculateWrappedDistance
  const distance = calculateWrappedDistance(hunter.position, lion.position);
