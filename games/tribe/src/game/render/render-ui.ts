...
function renderTooltip(ctx: CanvasRenderingContext2D, text: string, mousePosition: Vector2D): void {
  ctx.save();
  ...
  const boxY = mousePosition.y + UI_TOOLTIP_OFFSET_Y - boxHeight;
...
export function renderUIButtons(ctx: CanvasRenderingContext2D, gameState: GameWorldState, canvasWidth: number): void {
  ...
  if (hoveredButton && hoveredButton.tooltip && gameState.mousePosition) {
    renderTooltip(ctx, hoveredButton.tooltip, gameState.mousePosition);
  }
