...
const findBestFoodSource = (human: HumanEntity, context: UpdateContext): FoodSource | null => {
    ...
    const findFood = (radius?: number) => {
        ...
        if (closestBush && closestCorpse) {
            const distBush = calculateWrappedDistance(human.position, closestBush.position, gameState.mapDimensions.width, gameState.mapDimensions.height);
            const distCorpse = calculateWrappedDistance(human.position, closestCorpse.position, gameState.mapDimensions.width, gameState.mapDimensions.height);
