 ...
 */
export function createAnimalWanderBehavior(depth: number): BehaviorNode {
  return new ActionNode(
    (animal: any, context: UpdateContext, blackboard) => {
      ...
      if (!wanderTarget || !wanderStartTime || (currentTime - wanderStartTime) > 2) {
        ...
        const targetPosition = vectorAdd(
          animal.position,
          vectorScale(wanderDirection, distance)
        );
