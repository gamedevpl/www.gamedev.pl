/**
 * Simplified performance profiling script
 * Tests core game logic without UI/sound dependencies
 */

// Mock sound files to avoid import errors
const mockSound = '';
const modules = {
  './sound-effects/attack.mp3': mockSound,
  './sound-effects/gather.mp3': mockSound,
  './sound-effects/eat.mp3': mockSound,
  './sound-effects/procreate.mp3': mockSound,
  './sound-effects/birth.mp3': mockSound,
  './sound-effects/child-fed.mp3': mockSound,
  './sound-effects/human-death.mp3': mockSound,
  './sound-effects/game-over.mp3': mockSound,
  './sound-effects/button-click.mp3': mockSound,
  './music/soundtrack-1.mp3': mockSound,
};

// Mock module resolution for sound files
...
const { updateWorld } = require('../src/game/world-update');
const { profiler } = require('../src/game/performance-profiler');
const { createHuman } = require('../src/game/entities/entities-update');

function createManyHumans(gameState: any, count: number): void {
...
function runPerformanceTest(): void {
  ...
  const totalProfiledTime = topEntries.reduce((sum: number, [_, entry]: any) => sum + entry.duration, 0);
