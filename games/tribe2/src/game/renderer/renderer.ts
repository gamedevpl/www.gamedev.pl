  ...
  if (!gameState.wireframeMode) {
    ...
    if (false) { // gameState.roadEditingMode
      ...
      if (lastRoadPosition && previewRoadPosition) {
        // Calculate shortest path vector accounting for wrapping
        let dx = previewRoadPosition.x - lastRoadPosition.x;
        let dy = previewRoadPosition.y - lastRoadPosition.y;
        ...
        // Get all wrapped instances of the starting point
        const wrappedLastPositions = getWrappedEntityPositions(
          lastRoadPosition,
          viewportCenter,
          gameState.mapDimensions,
        );
        ...
        wrappedLastPositions.forEach((lastPos) => {
          const previewPos = { x: lastPos.x + dx, y: lastPos.y + dy };

          const screenLastPos = projectToScreen(lastPos, viewportCenter, viewportZoom, canvasDimensions);
          const lastHeight = getHeightAtWorldPos(
            lastPos,
            gameState.heightMap,
            HEIGHT_MAP_RESOLUTION,
            gameState.mapDimensions,
          );
          screenLastPos.y -= lastHeight * CANVAS_HEIGHT_DISPLACEMENT * viewportZoom;

          const screenPreviewPos = projectToScreen(previewPos, viewportCenter, viewportZoom, canvasDimensions);
          const previewHeight = getHeightAtWorldPos(
            previewPos,
            gameState.heightMap,
            HEIGHT_MAP_RESOLUTION,
            gameState.mapDimensions,
          );
          screenPreviewPos.y -= previewHeight * CANVAS_HEIGHT_DISPLACEMENT * viewportZoom;
