...
const TAKEOVER_TARGET_KEY = 'takeOverBuildingTarget';

/**
 * Creates a behavior that allows AI tribe leaders to autonomously take over enemy buildings.
 * 
 * The behavior:
 ...
 * 4. Initiates the takeover action when in range
 * 
 ...
 */
export function createTakeOverBuildingBehavior(depth: number): BehaviorNode<HumanEntity> {
  const takeOverAction = new Sequence(
    [
      // 1. Check if we can find an enemy building to take over
      new ConditionNode(
        (human: HumanEntity, context: { gameState: GameWorldState }, aiBlackboard) => {
          ...
          const existingTarget = Blackboard.get<EntityId>(aiBlackboard, TAKEOVER_TARGET_KEY);
          if (existingTarget) {
            ...
            Blackboard.delete(aiBlackboard, TAKEOVER_TARGET_KEY);
          }
          ...
          if (enemyBuilding) {
            Blackboard.set(aiBlackboard, TAKEOVER_TARGET_KEY, enemyBuilding.id);
        ...
        'Find Enemy Building to Take Over',
        depth + 2,
      ),
      ...
      new ActionNode<HumanEntity>(
        (human, context, aiBlackboard) => {
          const targetId = Blackboard.get<EntityId>(aiBlackboard, TAKEOVER_TARGET_KEY);
      ...
      // 3. Initiate the takeover action
      new ActionNode(
        (human: HumanEntity, context: { gameState: GameWorldState }, aiBlackboard) => {
          const targetId = Blackboard.get<EntityId>(aiBlackboard, TAKEOVER_TARGET_KEY);
        ...
        'Initiate Building Takeover',
        depth + 2,
      ),
    ],
    'Take Over Building Action',
    depth + 1,
  );
