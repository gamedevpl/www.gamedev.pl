 ...
 *
 * This generates code that uses the scenario scripting system.
 */
export function exportScenarioAsScriptTypeScript(config: ScenarioConfig): string {
  const lines: string[] = [];
  const scenarioVarName = sanitizeName(config.name);

  lines.push(`/**`);
  lines.push(` * Scenario Script: ${config.name}`);
  if (config.description) {
    lines.push(` * ${config.description}`);
  }
  lines.push(` *`);
  lines.push(` * Generated by Scenario Editor`);
  lines.push(` * This file contains both the world initialization and scenario scripting.`);
  lines.push(` */`);
  lines.push(``);
  lines.push(`// World initialization imports`);
  lines.push(`import { createEntities, createBerryBush, createHuman, createPrey, createPredator, createBuilding } from './entities/entities-update';`);
  lines.push(`import { GameWorldState } from './world-types';`);
  lines.push(`import { generateRandomPreyGeneCode } from './entities/characters/prey/prey-utils';`);
  lines.push(`import { generateRandomPredatorGeneCode } from './entities/characters/predator/predator-utils';`);
  lines.push(`import { BuildingType } from './entities/buildings/building-types';`);
  lines.push(``);
  lines.push(`// Scenario scripting imports`);
  lines.push(`import {`);
  lines.push(`  ScenarioScript,`);
  lines.push(`  createPlayerDeathFailCondition,`);
  lines.push(`  createPopulationWinCondition,`);
  lines.push(`  createSurvivalWinCondition,`);
  lines.push(`  createPopulationMetric,`);
  lines.push(`  createDaysSurvivedMetric,`);
  lines.push(`  createLowFoodNotificationTrigger,`);
  lines.push(`  createPredatorNearbyNotificationTrigger,`);
  lines.push(`} from './scenarios/scenario-script-types';`);
  lines.push(`import { NotificationType } from './notifications/notification-types';`);
  lines.push(`import { addNotification } from './notifications/notification-utils';`);
  lines.push(``);

  // Count entities for metrics
  const humanCount = config.tribes.reduce((sum, t) => sum + t.humans.length, 0);
  const predatorCount = config.predators.length;

  lines.push(`// ============================================================================`);
  lines.push(`// WORLD INITIALIZATION`);
  lines.push(`// ============================================================================`);
  lines.push(``);
  lines.push(`/**`);
  lines.push(` * Initializes the world state for this scenario.`);
  lines.push(` */`);
  lines.push(`export function initScenario_${scenarioVarName}(): GameWorldState {`);
  lines.push(`  const entities = createEntities();`);
  lines.push(`  const initialTime = 0;`);
  lines.push(``);
  ...
  // Generate berry bushes
  if (config.berryBushes.length > 0) {
    lines.push(`  // Create berry bushes (${config.berryBushes.length})`);
    for (const bush of config.berryBushes) {
      lines.push(`  createBerryBush(entities, { x: ${bush.position.x}, y: ${bush.position.y} }, initialTime);`);
    }
    lines.push(``);
  ...
  // Generate prey
  if (config.prey.length > 0) {
    lines.push(`  // Create prey (${config.prey.length})`);
    for (const prey of config.prey) {
      lines.push(`  createPrey(entities, { x: ${prey.position.x}, y: ${prey.position.y} }, '${prey.gender}', undefined, undefined, generateRandomPreyGeneCode());`);
    }
    lines.push(``);
  ...
  // Generate predators
  if (config.predators.length > 0) {
    lines.push(`  // Create predators (${config.predators.length})`);
    for (const predator of config.predators) {
      lines.push(`  createPredator(entities, { x: ${predator.position.x}, y: ${predator.position.y} }, '${predator.gender}', undefined, undefined, generateRandomPredatorGeneCode());`);
    }
    lines.push(``);
  ...
  // Generate buildings
  if (config.buildings.length > 0) {
    lines.push(`  // Create buildings (${config.buildings.length})`);
    for (const building of config.buildings) {
      const ownerVarName = `tribe_${sanitizeName(building.tribeId)}_leader`;
      const buildingTypeKey = getBuildingTypeKey(building.type);
      lines.push(`  const building_${sanitizeName(building.id)} = createBuilding(`);
  ...
  lines.push(`  // Return the initial world state (requires full initialization - see world-init.ts)`);
  lines.push(`  return {`);
  lines.push(`    time: initialTime,`);
  lines.push(`    entities,`);
  lines.push(`    mapDimensions: { width: ${config.mapWidth}, height: ${config.mapHeight} },`);
  ...
  lines.push(`    // ... add other required GameWorldState fields from world-init.ts`);
  lines.push(`  } as unknown as GameWorldState;`);
  lines.push(`}`);
  lines.push(``);
  ...
  lines.push(``);
  lines.push(`/**`);
  lines.push(` * Scenario script with win/fail conditions, progress tracking, and notifications.`);
  lines.push(` *`);
  lines.push(` * Customize the conditions and metrics below to define your scenario's objectives.`);
  lines.push(` */`);
  ...
  lines.push(`  // ========================================`);
  lines.push(`  // Define conditions that trigger victory.`);
  lines.push(`  // Any one condition being met ends the game with a win.`);
  lines.push(`  winConditions: [`);
  lines.push(`    // Example: Win when tribe population reaches 20`);
  lines.push(`    // createPopulationWinCondition(20),`);
  lines.push(``);
  lines.push(`    // Example: Win when surviving 30 days`);
  lines.push(`    // createSurvivalWinCondition(30 * 24), // 30 days in hours`);
  lines.push(``);
  lines.push(`    // Custom win condition:`);
  lines.push(`    // (state) => {`);
  lines.push(`    //   // Your custom logic here`);
  lines.push(`    //   return { triggered: false }; // or { triggered: true, message: 'Victory!' }`);
  lines.push(`    // },`);
  lines.push(`  ],`);
  lines.push(``);
  lines.push(`  // ========================================`);
  lines.push(`  // FAIL CONDITIONS`);
  lines.push(`  // ========================================`);
  lines.push(`  // Define conditions that trigger game over.`);
  lines.push(`  // Any one condition being met ends the game with a loss.`);
  lines.push(`  failConditions: [`);
  lines.push(`    // Default: Player death ends the game`);
  lines.push(`    createPlayerDeathFailCondition(),`);
  lines.push(``);
  lines.push(`    // Custom fail condition:`);
  lines.push(`    // (state) => {`);
  lines.push(`    //   // Your custom logic here`);
  lines.push(`    //   return { triggered: false }; // or { triggered: true, message: 'Game Over!' }`);
  lines.push(`    // },`);
  lines.push(`  ],`);
  lines.push(``);
  lines.push(`  // ========================================`);
  ...
  lines.push(`  // ========================================`);
  lines.push(`  // Define metrics to track and display to the player.`);
  lines.push(`  progressMetrics: [`);
  lines.push(`    createDaysSurvivedMetric(),`);
  lines.push(`    createPopulationMetric(),`);
  lines.push(``);
  lines.push(`    // Custom metric:`);
  lines.push(`    // {`);
  lines.push(`    //   id: 'custom-metric',`);
  lines.push(`    //   name: 'Custom Metric',`);
  lines.push(`    //   description: 'Description of the metric',`);
  lines.push(`    //   icon: '📊',`);
  lines.push(`    //   targetValue: 100, // Optional target`);
  lines.push(`    //   getValue: (state) => {`);
  lines.push(`    //     // Calculate and return the metric value`);
  lines.push(`    //     return 0;`);
  lines.push(`    //   },`);
  lines.push(`    //   formatValue: (value, target) => \`\${value}/\${target}\`,`);
  lines.push(`    // },`);
  lines.push(`  ],`);
  lines.push(``);
  lines.push(`  // ========================================`);
  ...
  lines.push(`  // ========================================`);
  lines.push(`  // Define custom notifications that trigger based on game events.`);
  lines.push(`  notificationTriggers: [`);
  lines.push(`    createLowFoodNotificationTrigger(),`);
  if (predatorCount > 0) {
    lines.push(`    createPredatorNearbyNotificationTrigger(200),`);
  }
  lines.push(``);
  lines.push(`    // Custom notification:`);
  lines.push(`    // {`);
  lines.push(`    //   id: 'custom-notification',`);
  lines.push(`    //   type: NotificationType.Hello,`);
  lines.push(`    //   message: 'Custom notification message',`);
  lines.push(`    //   duration: 24, // Game hours`);
  lines.push(`    //   triggerOnce: true, // Only trigger once`);
  lines.push(`    //   shouldTrigger: (state, previouslyTriggered) => {`);
  lines.push(`    //     // Return true when notification should appear`);
  ...
  lines.push(``);
  lines.push(`  /**`);
  lines.push(`   * Called once when the scenario starts.`);
  lines.push(`   */`);
  ...
  lines.push(``);
  lines.push(`  /**`);
  lines.push(`   * Called every game update tick.`);
  lines.push(`   * Use sparingly for performance.`);
  lines.push(`   */`);
  lines.push(`  // onUpdate: (state, deltaTime) => {`);
  lines.push(`  //   // Your per-frame logic here`);
  lines.push(`  // },`);
  lines.push(``);
  lines.push(`  /**`);
  lines.push(`   * Called when the scenario ends.`);
  lines.push(`   */`);
  lines.push(`  onEnd: (state, result, message) => {`);
  lines.push(`    console.log(\`Scenario ended: \${result} - \${message}\`);`);
  lines.push(`  },`);
  lines.push(`};`);
  lines.push(``);
  lines.push(`// ============================================================================`);
  ...
  lines.push(`// ============================================================================`);
  lines.push(`// Initial entities:`);
  lines.push(`//   - Tribes: ${config.tribes.length}`);
  lines.push(`//   - Humans: ${humanCount}`);
  lines.push(`//   - Berry Bushes: ${config.berryBushes.length}`);
  lines.push(`//   - Prey: ${config.prey.length}`);
  lines.push(`//   - Predators: ${config.predators.length}`);
  lines.push(`//   - Buildings: ${config.buildings.length}`);
  lines.push(`//   - Map Size: ${config.mapWidth}x${config.mapHeight}`);
