...
import { PLAYER_HIGHLIGHT_COLOR, FAMILY_CLAIM_COLOR, NON_FAMILY_CLAIM_COLOR } from '../ui/ui-consts.ts';
...
export function renderBerryBush(
  ...
  if (ownerId && claimedUntil && claimedUntil > time && player) {
    const owner = gameState.entities.entities[ownerId] as HumanEntity | undefined;
    if (owner) {
