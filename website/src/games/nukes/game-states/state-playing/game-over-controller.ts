...
export function GameOverController({
  ...
  useEffect(() => {
    ...
    const numStatesWithPopulation = Object.values(statePopulations).filter(
    ...
    const hostileStates = worldState.states.filter(
      (state) =>
        ...
        Object.entries(state.strategies).filter(
          ([stateId, strategy]) =>
            statePopulations[stateId] > 0 && strategy === Strategy.HOSTILE
        ).length > 0
    );
    const recentMissileLaunches = worldState.launchSites.some(
      (site) =>
        site.lastLaunchTimestamp &&
        currentTime - site.lastLaunchTimestamp < GAME_OVER_TIMEOUT
    );
    ...
    if (
      ...
      (!hostileStates.length &&
        !recentMissileLaunches &&
        currentTime > GAME_OVER_TIMEOUT)
    ) {
      const winner =
        numStatesWithPopulation === 1
          ? worldState.states.find((state) => statePopulations[state.id] > 0)
              ?.id
          : undefined;
      onGameOver({
        populations: Object.fromEntries(
          worldState.states.map((state) => [
            state.id,
            statePopulations[state.id],
          ])
        ),
        winner,
        stateNames: Object.fromEntries(
          worldState.states.map((state) => [state.id, state.name])
        ),
        playerStateId: worldState.states.find(
          (state) => state.isPlayerControlled
        )!.id,
      });
    }
  }, [worldState]);
