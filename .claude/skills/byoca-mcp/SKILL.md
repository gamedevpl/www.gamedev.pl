...
4. **Prefer `end` after the last successful `submit_sources`** if you will not deliver
   ...
   - Platform-managed rounds may start one automatic repair session after a red preview
     ...
     are never dispatched this way. Direct `GameKit.defineGame` errors in source or manifest
...
- **A frame that does not decode is refused too**, and separately: `frame_blank`, not
  `frame_shape`. A PNG signature and an IHDR measure fine, so the aspect-ratio check
  passes a header that draws nothing. `carriesPixels` therefore decodes rather than
  inspects: IHDR validated against the spec (colour type, bit depth, compression, filter,
  interlace), the IDAT chunks concatenated and inflated, and the output required to be
  exactly the scanlines the header promised — Adam7's seven passes included. Structural
  checks were tried first and were not enough; the repo's own test fixture had bit depth
  0 and junk IDAT bytes and passed them. The two refusals ask for different things — a
  reshaped frame needs resizing, one that will not decode needs drawing again — and the
  card outlives the claim, so an unrenderable frame is permanent. The proposal route
  repeats the check, for a frame stored before the upload guarded it.
  `pngjs` then decodes the frame properly — scanline filters, a palette for indexed
  colour, and the critical-chunk CRCs. It runs _after_ the checks above, never instead of
  them: `PNG.sync.read` accepts a PNG with no IDAT at all and hands back a zero-filled
  buffer, so substituting it would reopen the header-only hole. Each layer catches what
  the other cannot, which is why both are there.
  Decoding on an agent-reachable route is itself a surface: the inflate is capped at the
  pixels the header asks for, and a frame declaring more than 64 MB of them is refused
  before zlib or pngjs is called. 5 MB of zeros is a 4.9 KB upload. JPEG is still read
  structurally, not decoded; only the platform's own job accepts one.
...
and none of the success fields. Every tool's `outputSchema` admits that body through an `anyOf`
branch requiring only `error` — added centrally in `tools/list` by `withErrorBranch`, so a new tool
cannot forget it. Before that, a refusal failed the declared schema and clients raised
