...
- **An image-only service update does not pick up a new required env var, and a boot throw then wedges every later promotion.** Observed (#1464 review, 2026-09-24): world stopped falling back to `SESSION_SECRET` and exits unless `ZONE_TICKET_SECRET` is set. `deploy.yml` rebuilds the zone host when `apps/world` or `packages/zone-core` changes and runs `gcloud run services update --image` only, which keeps the previous secret map (`SESSION_SECRET` from `infra/deploy-world.sh`). The container never listens; `bash -e` aborts the workflow before app promotion; the next deploy retries the same roll. `deploy-world.sh` `--set-secrets` replaces the whole set, so a hand run of the old script drops a secret someone added with `--update-secrets`. When a PR removes a secret fallback, grep the workflow path list and the deploy script for an image-only update of that service, and require the new variable on the live service before merge. A PR note that says "do not deploy this service yet" is not a gate.
